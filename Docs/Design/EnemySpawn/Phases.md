...
### 구현 항목
- **연속적인 배율 조정**
  - 공식: `배율 = 1 - (현재 존재 점수 - 목표 존재 점수) / 목표 존재 점수`
  - 매 프레임 실시간 계산
  - 시간 기반 예산 증가율에 배율 곱셈 적용

- **범위 제한**
  - 예산 증가율 배율: 0.7x ~ 1.3x (70% ~ 130%)
  - Mathf.Clamp로 극단적 변화 방지

- **Inspector 설정**
  - `minBudgetRateMultiplier`: 최소 배율 (기본 0.7)
  - `maxBudgetRateMultiplier`: 최대 배율 (기본 1.3)

### 검증 방법
- 플레이어가 적을 빠르게 처리할 때 배율 증가 확인 (최대 1.3x)
- 플레이어가 압도당할 때 배율 감소 확인 (최소 0.7x)
- 균형 상태에서 배율 1.0x 확인
